...
#
# Returns:
# Exchange rate: 43.8077 COP/Bs.
# Effective Date: Lunes, 26 Mayo  2025


@pytest.mark.parametrize("debug", ["", "/0", "/2"])
def test_copveb_plain(client, debug):
    response = client.get(f'/copveb{debug}')
    assert response.status_code == 200
    assert response.text is not None
    assert 'Exchange rate:' in response.text
    assert 'COP/Bs.' in response.text
    assert 'Effective Date:' in response.text


# http://127.0.0.1:5001/copveb/1
#
# Returns:
...
# 'rublo': {'symbol': 'RUB', 'value': 1.19590782}, 'dolar': {'symbol':
# 'USD', 'value': 95.084}, 'effective_date': 'Lunes, 26 Mayo  2025',
# 'run_timestamp': '2025-05-25 11:11:35 UTC'}}
...
# '2025-05-25 11:11:36 UTC'}, 'official_cop': {'error': False,
# 'error_message': '', 'data': {'valor': '4165.41', 'unidad': 'COP',
# 'vigenciadesde': '2025-05-24T00:00:00.000', 'vigenciahasta':
# '2025-05-27T00:00:00.000', ':id': 'row-6z9a-eepe~rxcy', 'bank_value':
# '4165.41', 'bank_value_percent': 0.0}, 'run_timestamp':
# '2025-05-25 11:11:36 UTC'}}}


def test_copveb_debug(client):
    response = client.get('/copveb/1')
    assert response.status_code == 200
    assert response.text is not None
